...
#twitter http://twitter.com/oauth/authorize?oauth_nonce=7653375662207839052&oauth_timestamp=1293627634&oauth_consumer_key=MDdqlEqrONS7tWTLiFtvew&oauth_signature_method=HMAC-SHA1&oauth_version=1.0&oauth_token=Kg5aY1AIfSGYvHzNyyYeRXGyiZoqWVh17XJPznts&oauth_signature=leIDFJLZl0p7tEHv%2Fmib3s%2BdzHU%3D
...
ereporter.register_logger() #using ereporter
...
def twitter_specifier_handler(client):
    cre =  client.get('/account/verify_credentials')
    if cre:
...
class OAuthClient(object):
    ...
    def callback(self, return_to='/newuser'):
        ...
        if token_info and "Invalid / expired Token" in str(token_info):
            self.handler.redirect('/')
            return
    ...
    @staticmethod
    def cleanup():
        """clean old RequestToken"""
        query = OAuthRequestToken.all(keys_only=True).filter(
            'created <', datetime.datetime.now() - EXPIRATION_WINDOW
            )
        count = query.count(CLEANUP_BATCH_SIZE)
        db.delete(query.fetch(CLEANUP_BATCH_SIZE))
        return "Cleaned %i EXPIRED RequestToken" % count
